#$ -l tmem=6G,h_vmem=6G,h_rt=72:0:0,tscratch=4G
#$ -S /bin/bash
#$ -N bk_three_spec
#$ -R y
#$ -t 1-300
...
function finish {
    rm -rf /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
}
...
mkdir -p /scratch0/karkaria/BK_three_species_$SGE_TASK_ID/output

cd $HOME/cpp_consortium_sim

echo "copying inputfiles to tmpdir"
cp -R input_files /scratch0/karkaria/BK_three_species_$SGE_TASK_ID

cp ./*.py /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
cp ./*.cpp /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
cp ./*.h /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
cp ./*.so /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
cp -R exp_configs /scratch0/karkaria/BK_three_species_$SGE_TASK_ID

cd /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
pwd

echo "Running experiment"
/usr/bin/time --verbose python3 run_boost_rpr.py --config ./exp_configs/experiment_config_BK_manu_three_species_stable.yaml --exp_suffix $SGE_TASK_ID  >> $HOME/cpp_consortium_sim/output/log.BK_three_species_$SGE_TASK_ID
...
cp /scratch0/karkaria/BK_three_species_$SGE_TASK_ID/output/*.tar.gz $HOME/cpp_consortium_sim/output/
rm -r /scratch0/karkaria/BK_three_species_$SGE_TASK_ID
